            ...
            if model.isLoading {
                VStack(spacing: 10) {
                    ...
                    if model.canCancelInitialOpen {
                        ...
                        .help("Cancel opening the remote location")
                    }
                }
        ...
        VStack(alignment: .leading, spacing: 17) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                TextField("/srv/app/output", text: $model.remotePath)
                    ...
                    .accessibilityHint("Absolute path on the selected SSH host")
            ...
            HStack {
                Spacer()
                // While the open is pending, Cancel retires that request and
                // keeps the sheet, host, and path for another attempt; it
                // dismisses the sheet only when nothing is in flight.
                Button("Cancel") {
                    if isOpening, model.canCancelInitialOpen {
                        model.cancelInitialOpen()
                        isOpening = false
                    } else {
                        dismiss()
                    ...
                    .keyboardShortcut(.cancelAction)
                    .help(
                        isOpening && model.isLoading
                            ? "Cancel opening the remote path"
