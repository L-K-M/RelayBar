...
# build, and publish an unsigned (ad-hoc-signed) GitHub Release, like the
# sibling family apps (task 062). Unlike the sibling repos, CI does NOT inject
# the version at build time: Sparkle compares the committed build number, so
# the tag must match the committed MARKETING_VERSION (release.yml asserts this)
# and RELEASE_POST_BUMP increments the committed CURRENT_PROJECT_VERSION
# alongside every version bump. After CI publishes, follow the runbook
# (docs/system-specs/operations/build-and-release.md): the appcast, website,
# and cask steps stay manual and keychain-bound.
#
...
export RELEASE_CI_NOTE="CI (release.yml) will now test, build, and publish the unsigned (ad-hoc-signed) GitHub Release for <tag>, like the sibling family apps. Appcast/website/cask follow-ups stay manual — see docs/system-specs/operations/build-and-release.md."
