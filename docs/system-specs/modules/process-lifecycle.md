...
- Exhaustion changes the profile to failed and posts one system notification naming the profile and the failure (authorization requested lazily; denial is silent), unless this profile has already notified without reaching Running since. The message ends "RelayBar tries again when the network changes." because the profile stays wanted: it is started again by the next network path change, or by the user.
