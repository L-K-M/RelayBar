...
- The relaunch itself is never rationed, so a profile that fails for a reason no network can cure — a revoked key, say — still runs out of attempts, notifies, and is started once more by each later network change. It notifies once per dead streak: a repeat exhaustion with neither Running nor a fresh user start in between is silent.
...
- The pending network-change retry lives for the app's run; a quit ends it and the profile stays failed until the user starts it again. Start, stop, group Stop All, edit, and delete withdraw a profile from it. Restart All does not: an exhausted profile owns no lifecycle work, so it is neither relaunched nor withdrawn and stays armed for the next change. Profiles that failed for a configuration or preflight reason were never wanted by the process manager and are not started automatically.
