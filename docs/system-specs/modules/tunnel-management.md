...
- Named section headers keep persistent Start All and Stop All icon buttons at their trailing side, immediately before the section menu, without depending on pointer hover or menu focus. The section menu offers Restart All above rename and ungroup and does not duplicate Start All or Stop All. Enablement follows member state: Start All needs an inactive member, Stop All and Restart All need an active one. Targets are resolved by canonical group identity and snapshotted when the action begins; profiles outside the group are never affected.
