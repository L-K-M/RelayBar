...
- When the pasteboard holds a complete importable command, Quick Add offers a one-click (or ⇧⌘V) import chip; the clipboard is read only when the chip is clicked (never on editor appearance), partial or unrelated clipboard contents produce a gentle message, and typing hides the chip.
