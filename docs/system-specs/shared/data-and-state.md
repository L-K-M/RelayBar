...
- An existing `savedTunnels.v2` value that fails to decode is preserved verbatim under `savedTunnels.v2.corrupt-backup` before the store starts from an empty list, because the next save would otherwise overwrite the only copy of the user's profiles.
...
- Forwarding profiles and concrete aliases discovered from `~/.ssh/config` remain external inputs to the catalog. Config aliases are read on refresh and are not persisted as standalone RelayBar hosts. Within one catalog instance the parsed aliases are reused until the file's modification date or size changes, so repeated server refreshes cost one `stat` rather than a re-read.
