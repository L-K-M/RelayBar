...
- Imported and persisted options that execute commands, choose arbitrary config files, or write logs are blocked.
...
- Remote listings are capped at 10,000 supported entries, 32 KiB per line, and 4 KiB per entry name; negative sizes are rejected. Captured SFTP output is capped at 32 MiB, captured SFTP diagnostics at 1 MiB, image previews at 100 MiB, and Markdown previews at 2 MiB. Capture reads probe one byte beyond each SFTP cap so limit enforcement does not depend only on filesystem metadata, and exceeding either SFTP cap fails the operation even when the process exits successfully. Successful listing output must be readable and valid UTF-8; failed-command diagnostics are decoded loss-tolerantly only inside their existing byte bound.
...
- Upload opens the local source with no-follow and nonblocking flags, verifies
  the opened descriptor is a regular file, and snapshots it before SSH work.
  Copying uses bounded buffers and the opened size, checks cancellation, and
  preserves permission bits inside an atomically created `0700` directory.
  SFTP reads only that private snapshot, which is removed on upload exit; swapping
  the original pathname cannot redirect the upload. Concurrent writes to the
  opened inode are not excluded.
- Upload validates the complete remote path and uses a unique exact
  same-directory staging name. A new name
...
- Upload cleanup removes only the exact app-generated staging path, without a
  ...
  publication wins over a late cancellation. Cancellation after an unconfirmed
  publication attempt reports an unknown remote outcome, never inferred success
  from target existence. Each removal attempt, including detached recovery,
  has a ten-second deadline that cancels and reaps the child with the existing
  force-stop grace. Cleanup uncertainty preserves publication uncertainty.
  App termination waits for this bounded cleanup before retiring the SSH master.
